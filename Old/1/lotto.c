...
main() {
	...
	do {
		...
		if (choice == '1') {
			...
			if (guessStatistics.length != 0) {
				printf("teste %p", guessStatistics.numberRepeats);
				free(guessStatistics.numberRepeats);
				printf("%p", guessStatistics.numbersUsed);
...
void addGuessStats(stat * stats, int * guess) {
	...
	if (stats->length == 0) {
		...
		stats->numbersUsed = (int *) calloc(
			REC_NUMS_INCREMENT, sizeof(int)
		);
		
		stats->numberRepeats = (int *) calloc(
			REC_NUMS_INCREMENT, sizeof(int)
		);
